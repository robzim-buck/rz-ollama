...
def get_rag_chain_for_json(retriever_with_history):
    ...
    Do not limit your responses to 10 items. Include at least 25 items in your responses when asked for a list. \
...
def do_json_conversation():
    with open('zendesk_chatlog.txt', 'a') as ofile:
        my_retriever_with_history = get_retriever_with_history(retriever=get_vector_db_retriever_for_json(), llm=openai_llm_chat)
        my_rag_chain = get_rag_chain_for_json(retriever_with_history=my_retriever_with_history)
        myq = ''
        while myq != 'bye':
